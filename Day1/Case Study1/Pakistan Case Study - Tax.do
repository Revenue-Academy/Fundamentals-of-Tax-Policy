...
use "IMF Revenue Database March-2017 wide.dta", clear
...
replace inc=6.33 if year==2013 & Country_Code=="IND"
replace corp=3.95 if year==2013 & Country_Code=="IND"
replace indv=2.38 if year==2013 & Country_Code=="IND"
replace goods=5.80 if year==2013 & Country_Code=="IND"
...
replace inc=3.7 if year==2015 & Country_Code=="PAK"
replace excises=0.6 if year==2015 & Country_Code=="PAK"
replace genr=4.0 if year==2015 & Country_Code=="PAK"
replace trade=1.1 if year==2015 & Country_Code=="PAK"
replace propr=0.0 if year==2015 & Country_Code=="PAK"
...
replace genr=4.5 if year==2016 & Country_Code=="PAK" 
...
graph bar inc propr genr excises trade other_tax if Country_Code=="PAK", over(year, relabel(1 "1990" 2 " " 3 " " 4 " " 5 " " 6 "1995" 7 " " 8 " " 9 " " 10 " " 11 "2000" 12 " " 13 " " 14 " " 15 " " 16 "2005" 17 " " 18 " " 19 " " 20 " " 21 "2010" 22 " " 23 " " 24 " " 25 " " 26 " " 27 "2016")) legend(label(1 "Income Tax")  label(2 "Property Tax") label(3 "Tax on Goods") label(4 "Excises") label(5 "Tax on Intl. Trade") label(6 "Other Taxes")) stack
...
sort year
twoway (connected meantaxinc meantaxpropr meantaxgenr meantaxexcises meantaxtrade meantaxother_tax year if year<=2014 , yaxis(1) msymbol(Oh S T + - x) ytitle("% of GDP") ) (connected meantax year if year<=2014, yaxis(2) msymbol(o) title("Tax Structure over time"))
...
twoway (scatter tax ln_GDP_PC if year==2014, mlabsize(vsmall) mlabel(Country_Code)) (scatter tax ln_GDP_PC if year==2014 & Country_Code=="PAK", mcolor(red)) (lfit tax ln_GDP_PC if year==2014), graphregion(color(white)) bgcolor(white) ylabel(, nogrid) ytitle("Tax Revenues (% of GDP), 2014") xtitle("Log GDP per capita, 2014") title("Level of Income and Tax Revenues") legend(off)
...
drop if Country_Code=="LSO"
twoway (scatter tax agri_share if year==2014, mlabsize(vsmall) mlabel(Country_Code)) (scatter tax agri_share if year==2014 & Country_Code=="PAK", mcolor(red)) (lfit tax agri_share if year==2014), graphregion(color(white)) bgcolor(white) ylabel(, nogrid) ytitle("Tax Revenues (% of GDP), 2014") xtitle("Log GDP per capita, 2014") title("Level of Income and Tax Revenues") legend(off)

merge 1:m Country_Code year using "Polity Dataset Democracy"
drop if _merge !=3
drop _merge

twoway (scatter tax democracy if year==2014, mlabsize(vsmall) mlabel(Country_Code)) (scatter tax democracy if year==2014 & Country_Code=="PAK", mcolor(red)) (lfit tax democracy if year==2014), graphregion(color(white)) bgcolor(white) ylabel(, nogrid) ytitle("Tax Revenues (% of GDP), 2014") xtitle("Log GDP per capita, 2014") title("Level of Income and Tax Revenues") legend(off)
...
predict phat
list tax phat if year==2014 & Country_Code=="PAK"
drop phat

reg inc ln_GDP_PC agri_share democracy res_dum if year==2014
predict phat
list inc phat if year==2014 & Country_Code=="PAK"
drop phat

reg genr ln_GDP_PC agri_share democracy res_dum if year==2014
predict phat
list genr phat if year==2014 & Country_Code=="PAK"
drop phat

reg trade ln_GDP_PC agri_share democracy res_dum if year==2014
predict phat
list trade phat if year==2014 & Country_Code=="PAK"
drop phat

reg other_tax ln_GDP_PC agri_share democracy res_dum if year==2014
predict phat
list other_tax phat if year==2014 & Country_Code=="PAK"
drop phat
...
twoway (scatter resid_tax resid_lngdp if year==2014, mlabsize(vsmall) mlabel(Country_Code)) (scatter resid_tax resid_lngdp if year==2014 & Country_Code=="PAK", mcolor(red)) (lfit resid_tax resid_lngdp if year==2014), graphregion(color(white)) bgcolor(white) ylabel(, nogrid) ytitle("Tax Revenues (% of GDP), 2014", size (small)) xtitle("Log GDP per capita adjusted for democracy and resources, 2014", size (small)) title("Tax Revenues and Level of Income adjusted", size (small)) legend(off)

*time trends

graph bar inc propr genr excises trade other_tax if Country_Code=="PAK", over(year, relabel(1 "1990" 2 " " 3 " " 4 " " 5 " " 6 "1995" 7 " " 8 " " 9 " " 10 " " 11 "2000" 12 " " 13 " " 14 " " 15 " " 16 "2005" 17 " " 18 " " 19 " " 20 " " 21 "2010" 22 " " 23 " " 24 " " 25 " " 26 "2015" 27 "2016")) legend(label(1 "Income Tax")  label(2 "Property Tax") label(3 "Tax on Goods") label(4 "Excises") label(5 "Tax on Intl. Trade") label(6 "Other Taxes")) graphregion(color(white)) bgcolor(white) ylabel(, nogrid) ytitle("(% of GDP), 2014", size (small)) title("Pakistan Tax Structure (1990-2016)", size (small)) legend(off) stack
...
su buoy_cnt_tax_gt_1 if Country_Code=="PAK"
...
*sort Country_Code year
*twoway (lowess tax_buoyancy year if year<=2016 & Country_Code=="PAK", bwidth(0.2) msymbol(X)), title("Tax Buoyancy Pakistan (smoothed)")

twoway (connected tax_buoyancy year if year<=2016 & Country_Code=="PAK", yline(0) yline(1, lp(dash)) msymbol(X) ytitle("Multiples of GDP", size(small)) ), graphregion(color(white)) bgcolor(white) ylabel(, nogrid labsize(small)) xtitle(, size(small)) xlabel(, labsize(small)) text(2.6 2010 "`text1'", color(black) size(vsmall)) text(2.4 2010  "`text2'", color(black) size(vsmall)) title("Buoyancy of Overall Taxes in Pakistan", size(small))

*twoway (connected tax_buoyancy inc_buoyancy genr_buoyancy trade_buoyancy year if year<=2016 & Country_Code=="PAK", yline(0) yline(1, lp(dash)) msymbol(Oh S T + - x)), title("Tax Buoyancy Pakistan", size(small))

*twoway (connected tax_buoyancy year if year<=2016 & Country_Code=="PAK", msymbol(X) yline(0) yline(1, lp(dash)) ytitle("Multiples of GDP", size(small)) ylabel(-1.0(1.0)4.0)), graphregion(color(white)) bgcolor(white) ylabel(, nogrid labsize(small)) xtitle(, size(small)) xlabel(, labsize(small)) text(3.2 2011 "`text1'", color(black) size(vsmall)) text(3.0 2011  "`text2'", color(black) size(vsmall)) title("Buoyancy of Income Taxes in Pakistan", size(small))

twoway (connected inc_buoyancy year if year<=2016 & Country_Code=="PAK", msymbol(X) yline(0) yline(1, lp(dash)) ytitle("Multiples of GDP", size(small)) ylabel(-1.0(1.0)4.0)), graphregion(color(white)) bgcolor(white) ylabel(, nogrid labsize(small)) xtitle(, size(small)) xlabel(, labsize(small)) text(3.2 2011 "`text1'", color(black) size(vsmall)) text(3.0 2011  "`text2'", color(black) size(vsmall)) title("Buoyancy of Income Taxes in Pakistan", size(small))

twoway (connected genr_buoyancy year if year<=2016 & Country_Code=="PAK", msymbol(X) yline(0) yline(1, lp(dash)) ytitle("Multiples of GDP", size(small)) ylabel(-1.0(1.0)11.0)), graphregion(color(white)) bgcolor(white) ylabel(, nogrid labsize(small)) xtitle(, size(small)) xlabel(, labsize(small)) text(7.6 2011 "`text1'", color(black) size(vsmall)) text(7.0 2011  "`text2'", color(black) size(vsmall)) title("Buoyancy of Goods and Services Taxes in Pakistan", size(small))

twoway (connected trade_buoyancy year if year<=2016 & Country_Code=="PAK", msymbol(X) yline(0) yline(1, lp(dash)) ytitle("Multiples of GDP", size(small)) ylabel(-9.0(1.0)8.0)), graphregion(color(white)) bgcolor(white) ylabel(, nogrid labsize(small)) xtitle(, size(small)) xlabel(, labsize(small)) text(6.7 2011 "`text1'", color(black) size(vsmall)) text(6.0 2011  "`text2'", color(black) size(vsmall)) title("Buoyancy of Trade Taxes in Pakistan", size(small))

*TAX EFFICIENCY

merge 1:m Country_Code year using "Pakistan Tax Rate Data"
drop if _merge!=3
drop _merge
